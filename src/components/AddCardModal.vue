    ...
    <div class="bg-white p-6 rounded-xl w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <input type="text" v-model="form.cardHolderName" class="input" :class="errorClass('cardHolderName')" />
          <p v-if="errors.cardHolderName" class="text-red-500 text-xs mt-1">{{ errors.cardHolderName }}</p>
        </div>
        ...
        <div>
          ...
          <select v-model="form.cardNetwork" class="input" :class="errorClass('cardNetwork')" readonly>
            <option disabled value="">-- Select --</option>
            <option value="visa">Visa</option>
          </select>
          <p v-if="errors.cardNetwork" class="text-red-500 text-xs mt-1">{{ errors.cardNetwork }}</p>
        </div>
        ...
        <div>
          ...
          <input type="text" v-model="form.cvv" @input="filterNumberInput('cvv', $event)" maxlength="3" class="input" :class="errorClass('cvv')" />
...
const categories = ['My Debit Cards', 'All Company Cards', 'Personal Credit Cards', 'Team Expense Cards', 'Travel & Business Cards'];
...
function filterNumberInput(field: 'cardNumber' | 'cvv' | 'expiry', event: Event) {
  ...
  if (field === 'expiry') {
    ...
    if (onlyNumbers.length === 1) {
        form.expiry = onlyNumbers.charAt(0) === '0' || onlyNumbers.charAt(0) === '1' ? onlyNumbers : '';
    }
    if (onlyNumbers.length === 2) {
        form.expiry = onlyNumbers.charAt(0) === '1' && onlyNumbers.charAt(1) > '2' ? onlyNumbers.charAt(0): onlyNumbers;
    }
...
const validateForm = () => {
  ...
  errors.cardHolderName = !form.cardHolderName.trim() ?'Card holder name is required': (!/^[a-zA-Z\s]+$/.test(form.cardHolderName.trim()) ? 'Only letters and spaces are allowed' : '')
