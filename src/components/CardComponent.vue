<template>
  <div class="relative rounded-xl text-white p-6 h-full w-[95%] mt-7 mx-auto max-h-[248px]" :class="{
    ...
    <div class="flex items-center text-fs-14 font-open-sans font-bold tracking-wide leading-lh-19 mt-8 mb-5">
      <span class="mr-7 flex justify-between w-13" v-for="(set, index) in [[0,4],[4,8],[8,12],[12,16]]" :key="index">
        <span v-for="(i) in details.number.slice(...set)" :key="i"
          :class="{
...
const props = defineProps<{ list: Ref<CardDetail[]>, cardLoading: Ref<boolean>, categoryId: string, activeIndex: number }>();
