<template>
  <div class="bg-mint-cream">
    <div class="py-1 px-6 rounded-b-lg border-x border-b border-pale-gray bg-white">
      <div
        v-for="(tx, index) in transactions.slice(0, 4)"
        :key="index"
        class="py-5 border-b border-[#F2F2F2] last:border-none flex gap-3 items-start"
      >
        <div
          :class="[
            'w-12 h-12 rounded-full flex items-center justify-center shrink-0',
            getExpenseBg(tx.expenseType),
          ]"
        >
        ...
        <div class="flex flex-col flex-1">
          <div
            class="flex justify-between items-center font-opensans font-semibold text-fs-14 leading-lh-18"
          >
            ...
            >
              <span
                >{{ tx.amount > 0 ? '+ ' : '- ' }}S$
                {{ formattedAmount(tx.amount.toString()) }}</span
              >
              <img src="../assets/next.svg" alt="next" class="w-[6px] h-3 inline-block ml-2" />
            </span>
          </div>
          <div class="text-text-grey-1 font-opensans font-regular text-fs-13 leading-lh-18 mt-1">
            {{ tx.date }}
          </div>
          <div class="flex items-center mt-3 gap-2">
            ...
            <span class="text-text-blue-1 font-opensans font-semibold text-fs-12 leading-lh-18">{{
              tx.status === 'credit' ? 'Refund on debit card' : 'Charged to debit card'
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div
      class="text-text-green bg-mint-cream text-center font-opensans font-semibold text-fs-13 leading-lh-18 py-4 cursor-pointer border-x border-b border-pastel-mint"
    >
