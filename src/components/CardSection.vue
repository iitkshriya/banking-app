  ...
  return CardActionsList.map((action) => {
    switch (action.type) {
      case 'freeze':
        return {
          ...action,
          name: cardDetailsRef.value[activeCardRef.value?.currentSlide || 0]?.freezeStatus
            ? 'Unfreeze Card'
            : 'Freeze Card',
          action: () => {
            ...
            if (activeCardRef.value) {
              ...
              updateTabItems(
                currentCardId,
                { property: 'freezeStatus', value: null, type: 'boolean' },
                props.activeTab,
              )
            }
          },
...
onMounted(async () => {
  watch(() => activeCardRef.value?.currentSlide, fetchTransactions, { immediate: true })
  watch(() => props.cardList, fetchCardDetails, { immediate: true, deep: true })
})
</script>
